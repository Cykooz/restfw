...
class RestACLAuthorizationPolicy(object):
    ...
    @staticmethod
    def _get_context_permission(context, permission):
        """Returns context-aware version of given permission."""
        if context:
            method_options = getattr(context, 'options_for_%s' % permission, None)
            if method_options is not None and method_options.permission is not None:
                permission = method_options.permission
        return permission
    ...
    def principals_allowed_by_permission(self, context, permission):
        ...
        for location in reversed(list(lineage(context))):
            ...
            for ace_action, ace_principal, ace_permissions in acl:
                if not is_nonstr_iter(ace_permissions):
                    ace_permissions = [ace_permissions]
                is_match = self._match_permission(permission, ace_permissions)

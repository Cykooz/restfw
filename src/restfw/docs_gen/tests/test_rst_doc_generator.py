...
from ...tests.test_usage_examples_collector import Dummy1Examples, DummyContainerExamples, prepare_env
...
def test_doc_generator(web_app, app_config, tmpdir):
    ...
    assert sorted(os.listdir(restfw_app_dir)) == ['DummyContainer', 'DummyResource', 'index.rst']
    ...
    assert sorted(os.listdir(app_dir)) == ['DummyContainer', 'DummyResource', 'index.rst']

...
def get_input_data(
    context, request: PyramidRequest, schema: colander.SchemaNode
) -> Union[dict, list]:
    if not schema:
        return {}

    content_type = request.content_type
    if request.method in METHODS_WITH_BODY and content_type.startswith(
        'application/json'
    ):
        try:
...
def _create_error(
    node: colander.SchemaNode, message: str, child_node_path=None, value=None
) -> colander.Invalid:
    if child_node_path:
        pos = None
...
def create_multi_validation_error(schema_class, errors: Dict[str, str]):
    ...
    for node_name, message in errors.items():
        error.add(_create_error(schema, message, node_name))
    return colander_invalid_to_response(error)
...
def register_resource_links_extender(config: Configurator, adapter, resource_class):
    ...
    config.registry.registerAdapter(
        adapter, required=resource_class, provided=IHalResourceLinks
    )
